...
- `doc.md` v0.7 已按此定线：**GPIO10/14 → IN1(EN)**、**GPIO11/15 → IN2(PH)**
...
文档侧已在 §九「待核实」**#11** 立档，裁决后回写 `doc.md`。

### 5.3 CAN 波特率

`doc.md` 已在 §九 待定决策 **#4** 立档：固件默认 **500k 是占位值**。
...
| `doc.md` §五.4 | VREF = 10k+22k 分压（**无门控**） | **加 nSLEEP 门控**（P-MOS+NPN，v0.7 已写入文档与 BOM） | 原方案常态耗 106µA ≈ 0.42mW@24V，是待机预算 0.25mW 的 **1.7 倍** |
...
| 待机电流 | 最低（§六 的 0.25mW 按这个口径算） | 更高 ⇒ **0.25mW 要按实测重算** |
...
| 档 | 何时 | 待机 | 能不能被远程唤醒 |
|---|---|---|---|
| **深睡档** | `wl off`，或监听窗口超时回落 | ≈10µA ≈ **0.25mW@24V** | ❌ 只能本地（编码器 INT/RTC/干接点） |
