 ...
 * ⚠️ 门控后采样存在盲区(= 采样周期)。**采样周期由调用方决定**, 本模块只管单次采样:
 *    当前唯一的周期调用是 `power_state_tick()` (主循环 10Hz) ⇒ 盲区 **100ms**;
 *    LightSleep 档还要加上轻睡驻留时间, DeepSleep 档跨一次复位是 100~500ms。
 * ⚠️ **过压检测尚未实现** —— 本模块只给出读数, 没有任何过压阈值判断;
 *    doc.md §5.1 讨论过的"把采样交给 LP 核提到 ~100Hz (盲区 10ms)"那条路径
 *    在工程里没有对应代码。
